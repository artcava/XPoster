...
For Problem 1, the `SenderPlatform` split restores the invariant that was implicit in [ADR-003](ADR-003.md): a sender knows only how to publish to a platform, not what content strategy produced the post.

For Problem 2, the capability-interface model aligns with the direction already taken in [ADR-004](ADR-004.md): that concrete model names and vendor details are internal concerns of each AI implementation. Capability interfaces carry that principle one step further — a provider now also declares *what it can do*, not just *who it is*. This makes capability gaps explicit at compile time and eliminates the runtime branching in `HybridAiService` that currently silently degrades to a no-op.
...
*See also: [Architecture overview](../architecture.md) · [All ADRs](../index.md#analysis) · [Issue #133](https://github.com/artcava/XPoster/issues/133)*
