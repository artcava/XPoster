...
This guide explains how to obtain fal.ai API credentials and configure XPoster to use `FalAiImageService` as the image generation provider within `HybridAiService`.

> **Provider capabilities:** Image generation only (FLUX.2 Turbo)  
> **`AiProvider` enum value:** `DeepSeekWithFal` (paired with DeepSeek for text — see [setup-deepseek.md](setup-deepseek.md))
...
fal.ai is always paired with DeepSeek in `HybridAiService`. Set all required keys together:
...
> ℹ️ See [setup-deepseek.md](setup-deepseek.md) to obtain `DEEPSEEK_API_KEY`.
...
## 7. How fal.ai Fits in HybridAiService

`HybridAiService` delegates `GenerateImageAsync` exclusively to `FalAiImageService`:

| Operation | Routed to | Rationale |
|-----------|-----------|----------|
| `GetSummaryAsync` | `DeepSeekService` | Text task — fal.ai is not involved |
| `GetImagePromptAsync` | `DeepSeekService` | Text task — fal.ai is not involved |
| `GenerateImageAsync` | `FalAiImageService` | FLUX.2 Turbo delivers faster and cheaper images than OpenAI models for this workload |
...
- Review the prompt generated by `GetImagePromptAsync` in the Application Insights logs.
