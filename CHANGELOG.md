...
- **`Microsoft.Extensions.Http` added to `XPoster.csproj`** ([#133](https://github.com/artcava/XPoster/issues/133)): brings the Polly v8 standard resilience pipeline via the .NET 8 `AddStandardResilienceHandler` extension, without a direct `Polly` package reference.
...
- **`Program.cs`** updated to register `IKeyVaultService → KeyVaultService` as `Singleton`; all three sender constructors updated to accept the new dependency ([#113](https://github.com/artcava/XPoster/issues/113)).
- **`local.settings.json.example`** updated: all per-platform credential env vars (`IN_*`, `X_*`, `IG_*`) removed; `KEYVAULT_URI` added with inline instructions pointing to the Key Vault secret naming convention ([#113](https://github.com/artcava/XPoster/issues/113)).
...
### Fixed
- **`InSenderTests.SendAsync_WhenLinkedInOrgIdPresent_UsesOrgIdSecret` — wrong assertion** ([#113](https://github.com/artcava/XPoster/issues/113)): test renamed to `SendAsync_WhenLinkedInOrgIdPresent_UsesOrgIdAndSkipsOwnerCode`; assertion corrected from `Times.AtLeastOnce` on `LinkedInOwnerCode` to `Times.Never` — when `LinkedInOrgId` resolves successfully, `ResolveAuthorUrnAsync` uses it directly and never reads `LinkedInOwnerCode`.
...
### Tests
- **`KeyVaultServiceTests`** (`tests/XPoster.Tests/Services/KeyVaultServiceTests.cs`) ([#113](https://github.com/artcava/XPoster/issues/113)): verifies canonical secret name constants, transparent rotation scenario (second call after credential change returns updated value), and that no KV call is made when `SendAsync` receives a no-image post.
- **`InSenderTests`** updated ([#113](https://github.com/artcava/XPoster/issues/113)): `SendAsync_CalledTwice_QueriesKvAccessTokenOnEachCall` verifies KV is called on every invocation (rotation guarantee); `SendAsync_WhenLinkedInOrgIdPresent_UsesOrgIdAndSkipsOwnerCode` verifies `LinkedInOrgId` is used as author URN and `LinkedInOwnerCode` is never read when org secret resolves successfully.
