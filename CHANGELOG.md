...
- **X media upload APPEND rejected with HTTP 400 code 38 "media parameter is missing."** ([#297](https://github.com/artcava/XPoster/issues/297)): `XApiClient.AppendMediaSegmentsAsync` now sends each chunk as a `media` part in a `multipart/form-data` body, as required by the v1.1 media-upload APPEND command, instead of a bare `application/octet-stream` body. The `command` / `media_id` / `segment_index` query parameters remain signed; a multipart body is intentionally outside the OAuth 1.0a signature scope (RFC 5849 §3.4.1.3).
- **X media upload declared the wrong `media_type`**: `XSender` now detects the actual image format (SkiaSharp `SKCodec`) and passes the matching MIME type (`image/jpeg`, `image/png`, `image/gif`, `image/webp`) to the INIT command, instead of hardcoding `image/jpeg`. When the format cannot be detected, the sender logs a warning and falls back to a text-only tweet.
...
- **`FbSender`** (`src/SenderPlugins/FbSender.cs`): new `ISender` implementation publishing posts to a Facebook Page via the Meta Graph API; text-only posts use `/{page_id}/feed`, image posts use `/{page_id}/photos`; `Platform` returns `SenderPlatform.Facebook`; `MessageMaxLenght` returns `3000` ([#224](https://github.com/artcava/XPoster/issues/224))
...
- **`DefaultSlotProfileProvider`**: `SenderPlatform.Facebook` added to the hours 6 & 14 fan-out slots after staging validation ([#224](https://github.com/artcava/XPoster/issues/224))
- **`Program.cs`**: `AddTransient<FbSender>()` registered ([#224](https://github.com/artcava/XPoster/issues/224))
- **`CredentialsExtensions.cs`**: `AddCredentials` registered `FacebookCredentialsValidator` ([#224](https://github.com/artcava/XPoster/issues/224))
